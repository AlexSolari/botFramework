...
function createMockStorage(loadResponse: Record<number, ActionStateBase> = {}): IStorageClient {
    return {
...
function createChatContext(): ChatContextInternal<ActionStateBase, ScheduledAction<ActionStateBase>> {
    const storage = createMockStorage();
    const scheduler = createMockScheduler();
    const eventEmitter = new TypedEventEmitter();
    
    const ctx = new ChatContextInternal<ActionStateBase, ScheduledAction<ActionStateBase>>(storage, scheduler, eventEmitter);
...
describe('ChatContextInternal', () => {
    describe('constructor', () => {
        ...
        test('should have isInitialized as false by default', () => {
            ...
            const ctx = new ChatContextInternal<ActionStateBase>(storage, scheduler, eventEmitter);
    ...
    describe('capture controller', () => {
        ...
        test('captureReplies should create default AbortController if not provided', () => {
            ...
            const response = ctx.responses[0] as TextMessage;
            expect(response.captures[0].abortController).toBeInstanceOf(AbortController);
        });
    ...
    describe('loadStateOf', () => {
        test('should call storage.load with the action', async () => {
            const loadMock = mock(() => Promise.resolve({}));
            const storage: IStorageClient = {
                load: loadMock,
                close: mock(() => Promise.resolve()),
                getActionState: mock(() => Promise.resolve({ lastExecutedDate: 0, pinnedMessages: [] })),
                saveActionExecutionResult: mock(() => Promise.resolve()),
                updateStateFor: mock(() => Promise.resolve())
            } as unknown as IStorageClient;
            ...
            const otherAction = Object.assign(Object.create(Object.getPrototypeOf(baseAction) as object), baseAction, {
                key: 'scheduled:other-action' as ActionKey
            }) as ScheduledAction<ActionStateBase>;
            
            await ctx.loadStateOf(otherAction);
            
            expect(loadMock).toHaveBeenCalledWith(otherAction);
        });

        test('should return state for current chat if it exists', async () => {
            ...
            const result = await ctx.loadStateOf(otherAction);
            
            expect(result.lastExecutedDate).toBe(12345);
        });

        test('should return new state from constructor if no state exists', async () => {
            const storage: IStorageClient = {
        ...
        test('should return frozen state', async () => {
            const existingState = new ActionStateBase();
