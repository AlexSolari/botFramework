...
function createMessageContext(
    ...
    const ctx = new MessageContextInternal<ActionStateBase>(storage, scheduler, eventEmitter);
    ctx.isInitialized = true;
    ctx.action = createMockCommandAction();
    ctx.chatInfo = new ChatInfo(12345, 'Test Chat', []);
    ctx.traceId = 'trace-123' as TraceId;
    ctx.botName = 'TestBot';
    ctx.messageInfo = new MessageInfo(100, messageText, MessageType.Text, {} as Message);
    ctx.userInfo = new UserInfo(999, 'TestUser');
    ctx.botInfo = { id: 111, is_bot: true, first_name: 'Bot', username: 'testbot', can_join_groups: true, can_read_all_group_messages: false, supports_inline_queries: false, can_connect_to_business: false, has_main_web_app: false } as UserFromGetMe;
...
describe('MessageContextInternal', () => {
    ...
    describe('reply.andQuote.withText', () => {
        ...
        test('should include quote from match results when available', () => {
            ...
            ctx.matchResults = [['test 123', '123'] as unknown as RegExpMatchArray];
