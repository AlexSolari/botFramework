...
function createMockTelegramBot(): MockTelegramBot {
    ...
    return {
        ...
        getOnLastArgs: () => onMock.mock.calls.at(-1) as [string, InlineQueryEventHandler] | undefined,
...
function createMockInlineQueryAction(
    ...
    const execMock: Mock<(ctx: unknown) => Promise<BotResponse[]>> = mock(() => Promise.resolve(execResult));
    return {
...
describe('InlineQueryActionProcessor', () => {
    ...
    describe('initialize', () => {
        test('should not create task when no inline queries', () => {
            ...
            processor.initialize(
                mockApi, 
                mockTelegram as unknown as Parameters<typeof processor.initialize>[1], 
                [], 
                100 as Milliseconds
            );
        ...
        test('should set up message handler when inline queries provided', () => {
            const mockApi = createMockTelegramApi();
            const mockTelegram = createMockTelegramBot();
            const mockInlineAction = createMockInlineQueryAction('test-inline', /test/);
        ...
        test('should store inline queries for processing', () => {
            const mockApi = createMockTelegramApi();
            const mockTelegram = createMockTelegramBot();
            const mockInlineAction1 = createMockInlineQueryAction('test-inline-1', /test1/);
            const mockInlineAction2 = createMockInlineQueryAction('test-inline-2', /test2/);
    ...
    describe('edge cases', () => {
        test('should handle no inline query actions', () => {
            ...
            localProcessor.initialize(
                mockApi,
                mockTelegram as unknown as Parameters<typeof localProcessor.initialize>[1],
                [],  // No inline actions
                100 as Milliseconds
            );
    ...
    describe('query processing', () => {
        test('should emit processing started event when query received', () => {
            ...
            localEventEmitter.on(BotEventType.inlineProcessingStarted, (_ts, data) => {
            ...
            const mockInlineAction = createMockInlineQueryAction('query-action', /query/);
            ...
            const inlineQueryHandler = mockTelegram.getEventHandler('inline_query');
        ...
        test('should emit abort event when second query from same user received', () => {
            ...
            localEventEmitter.on(BotEventType.inlineProcessingAborting, (_ts, data) => {
            ...
            const mockInlineAction = createMockInlineQueryAction('abort-action', /abort/);
        ...
        test('should filter pending queries when receiving duplicate user query', () => {
            ...
            const mockInlineAction = createMockInlineQueryAction('filter-action', /filter/);
